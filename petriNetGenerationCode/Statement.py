...
class Statement:
    def __init__(self, subject, modalVerb, verb, object, receiver, valid=True) -> None:
    ...
    def fulfillmentCondition(self) -> ActionCondition:
        ...
        return ActionCondition(subject=self._subject,
                               ...
                               object=self._object, receiver=self._receiver)
    ...
    def receiver(self) -> Receiver:
        """Receiver of statement"""
        return Receiver.get_receiver(self._receiver)

    def label(self) -> str:
        """Label of statement (for Petri Net)"""
        return f"{self.subject().label()} {self.modalVerb().value} {self.verb().value} {self.object().label()} {self.receiver().label()}"
...
class TemporalStatement(Statement):
    def __init__(self, subject, modalVerb, verb, object, receiver, temporalExpression, valid=True):
        super().__init__(subject, modalVerb, verb, object, receiver, valid)
        self._temporalExpression = temporalExpression
        self._fulfilled_cond = self.fulfillmentCondition()

    def label(self):
        return f"{self.subject().label()} {self.modalVerb().value} {self.verb().value} {self.object().label()} {self.receiver().label()} {self.temporalExpression().label()} "
    ...
    def fulfillmentCondition(self):
        return TemporalActionCondition(subject=self._subject,
                                       verb_status=Verb.get_verb_status(self._verb).value,
                                       object=self._object, receiver=self._receiver, temporalExpression=self.temporalExpression())
